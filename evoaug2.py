...
# Example usage and comparison with original RobustModel
def main():
    """
    Example showing how to use the new DataLoader implementation.
    """
    ...
    class SimpleGenomicDataset(Dataset):
        def __init__(self, num_samples=1000, seq_length=200, num_classes=2):
            self.sequences = torch.randint(0, 4, (num_samples, seq_length))
            self.sequences = torch.nn.functional.one_hot(self.sequences, num_classes=4).float()
    ...
    # Create datasets
    train_dataset = SimpleGenomicDataset(num_samples=1000, seq_length=200)
    val_dataset = SimpleGenomicDataset(num_samples=200, seq_length=200)
    
    # Define augmentations (same as original EvoAug)
    augment_list = [
        RandomDeletion(delete_min=0, delete_max=20),
        RandomRC(rc_prob=0.5),
    ...
    # Create dataloaders
    train_dataloader = AugmentedGenomicDataLoader(
    ...
    # NOTE: can choose which splits to augment
    ...
    # Training loop with augmentations
    ...
    # Finetuning without augmentations - no need to provide augment_list
    print("\n=== Finetuning without augmentations ===")
    finetune_dataloader = AugmentedGenomicDataLoader(
        base_dataset=train_dataset,
        batch_size=32
    )
    ...
    for epoch in range(2):
        for batch_idx, (sequences, targets) in enumerate(finetune_dataloader):
            optimizer.zero_grad()
            outputs = model(sequences)
            loss = criterion(outputs, targets)
            loss.backward()
