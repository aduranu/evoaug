"""
PyTorch DataLoader implementation of EvoAug augmentations.

This module is the v2 entry point: a drop-in DataLoader that applies
length-preserving augmentations on-the-fly, replacing the v1
``RobustModel`` Lightning wrapper (still available under
``evoaug.legacy``).

``RobustLoader`` inherits from ``torch.utils.data.DataLoader`` and can be
used directly in PyTorch Lightning DataModules or vanilla PyTorch
training loops.
...
class RobustLoader(DataLoader):
    """
    EvoAug DataLoader that inherits from PyTorch DataLoader.
